...
const STOCK_PHOTOS={"Chicken": "https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Beef & Lamb": "https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Pork": "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Seafood": "https://images.unsplash.com/photo-1565557623262-b51c2513a641?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Vegetarian": "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600&sat=-10", "Soups & Stews": "https://images.unsplash.com/photo-1603105037880-880cd4edfb0d?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Salads & Sides": "https://images.unsplash.com/photo-1540420773420-3366772f4999?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Pasta & Noodles": "https://images.unsplash.com/photo-1473093295043-cdd812d0e601?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Handhelds & Grilling": "https://images.unsplash.com/photo-1555939594-58d7cb561ad1?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Desserts": "https://images.unsplash.com/photo-1578985545062-69928b1d9587?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Collections & Other": "https://images.unsplash.com/photo-1498837167922-ddd27525d352?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1600", "Featured": "https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=84&w=1800", "Family Favorites": "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1800", "Seasonal": "https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=82&w=1800"};
...
function sourcePreviewForRecipe(r){
  const params=new URLSearchParams({url:r.url,meta:'false',embed:'image.url'});
  return `https://api.microlink.io/?${params.toString()}`;
}
function handleRecipeImageError(img){
  const wrap=img.closest('.card-thumb');
  if(wrap){
    wrap.classList.add('no-photo');
    wrap.innerHTML='<div class="no-photo-mark">Recipe</div>';
  }
}

function photoForCategory(category){
  return STOCK_PHOTOS[category] || STOCK_PHOTOS["Collections & Other"];
}
...
function recipeCard(r){const fav=state.favorites.includes(r.id),note=state.notes[r.id]||'',rating=state.ratings[r.id]||'',ings=state.ingredients[r.id]||'';return `<article class="card"><div class="card-thumb"><img src="${sourcePreviewForRecipe(r)}" loading="lazy" referrerpolicy="no-referrer" alt="${esc(r.title)} source preview image" onerror="this.onerror=null;handleRecipeImageError(this)"></div><div class="card-main"><div class="badges">${[r.category,...r.tags.slice(0,2)].map(x=>`<span class="badge">${esc(x)}</span>`).join('')}</div><h3>${esc(r.title)}</h3><div class="source">${esc(r.source)} · ${esc(r.protein)}</div></div><div class="actions"><a class="open" href="${esc(r.url)}" target="_blank" rel="noopener" data-open="${r.id}">Open Recipe ↗</a><button class="icon-btn ${fav?'active':''}" data-favorite="${r.id}">${fav?'★':'☆'}</button><button class="icon-btn" data-plan="${r.id}">＋</button></div><details><summary>Notes, ingredients, and rating</summary><textarea placeholder="My notes…" data-note="${r.id}">${esc(note)}</textarea><textarea placeholder="Searchable ingredients, separated by commas…" data-ingredients="${r.id}">${esc(ings)}</textarea><div class="rating-row"><label>Rating</label><select data-rating="${r.id}"><option value="">Not rated</option>${[1,2,3,4,5].map(n=>`<option value="${n}" ${String(rating)===String(n)?'selected':''}>${'★'.repeat(n)}</option>`).join('')}</select></div></details></article>`}
...
function setFeatured(r){featuredTitle.textContent=r.title;featuredMeta.textContent=`${r.source} · ${r.category}`;featuredLink.href=r.url;featuredLink.dataset.featuredId=r.id;featuredImage.src='https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=84&w=1800';featuredImage.alt='Featured dinner table photograph'}
